...
class MusicDirectory(PySide6.QtCore.QAbstractTableModel):
	...
	def __init__(self, parent=None):
		...
		self.column_fields = ["title", "author", "duration", "bpm", "comment", "last_played", "age"]

		self._directory = ""
		prefs = preferences.Preferences.getInstance()
		if not prefs.has("directory/sort_order"):
			prefs.add("directory/sort_order", ["bpm", "last_played", "age", "title", "duration", "author", "comment"])  # You can sort multiple fields at the same time. These two lists are in order of priority.
		if not prefs.has("directory/sort_direction"):
			prefs.add("directory/sort_direction", [False, False, False, False, False, False, False])  # For each sort order, whether it is descending (True) or ascending (False).
		...
		if not prefs.has("directory/column_width"):
			...
			prefs.add("directory/column_width", [fraction, fraction, fraction, fraction, fraction, fraction, fraction])
	...
	def headerData(self, section, orientation, role=PySide6.QtCore.Qt.DisplayRole):
		...
		if orientation == PySide6.QtCore.Qt.Orientation.Horizontal:
			return ["title", "author", "duration", "bpm", "comment", "last_played", "age"][section]
